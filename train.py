...
def run_training(
    ...
    train_data = MVTecAT(dataset_path, data_type, transform=train_transform,
    ...
    for step in tqdm(range(epochs)):
        ...
        if test_epochs > 0 and epoch % test_epochs == 0:
            ...
            model.eval()
            roc_auc = eval_model(dataset_path, model_name, data_type, device=device,
                                 save_plots=False,
                                 size=size,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--head_layer', default=1, type=int,
                        help='number of layers in the projection head (default: 1)')  # head有多少个全连接层
    ...
    print(args)
    all_jizhong = os.listdir(args.dataset_path)   # 同时训练多个机种，每个机种下面又有多个工位数据集
    for cur_jizhong in all_jizhong:
        cur_jizhong_path = os.path.join(args.dataset_path, cur_jizhong)
        all_types = os.listdir(cur_jizhong_path)
        # all_types = ['lensbase',
        #              'taoqikong',
        #              'tongguangkong'
        #              ]
        ...
        # create modle dir
        args.model_dir = os.path.basename(cur_jizhong_path)
